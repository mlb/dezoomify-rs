...
fn resolve_bulk_output_file(args: &Arguments) -> Option<std::path::PathBuf> {
...
fn create_single_url_args(
    ...
    url: &str,
    index: usize,
    bulk_output_file: &Option<std::path::PathBuf>,
) -> Arguments {
    ...
    if base_args.is_bulk_mode() && !base_args.has_level_specifying_args() {
...
fn handle_single_url_result(
    result: Result<std::path::PathBuf, ZoomError>,
    index: usize,
    total_urls: usize,
...
mod tests {
    use super::*;
    use dezoomify_rs::{Arguments, ZoomError};
    use std::path::Path;
    ...
    fn test_create_single_url_args() {
        ...
        let bulk_output_file = Some(std::path::PathBuf::from("output.jpg"));
        ...
        assert_eq!(single_args.parallelism, 8); // Preserved from base
        assert!(single_args.largest); // Should be true when no level args
        assert_eq!(
            single_args.outfile,
    ...
    fn test_create_bulk_error_result() {
        ...
        assert!(create_bulk_error_result(1).is_err());
        assert!(create_bulk_error_result(5).is_err());
    }
    ...
    fn test_handle_single_url_result() {
        ...
        // Test partial download (counts as success)
